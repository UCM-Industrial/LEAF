...
def fill_df_pattern(df,df_pattern):#FILL pattern DATAFRAME
    ...
    for energy in energys2:
        if energy != "Nuclear" and energy != "Gas":
            df2 = df[energy]
...
if __name__ == '__main__':
    with open(sys.argv[1], "r",encoding="utf-8") as ymlfile:
        ...
        if data_yml.get("variability", False):
            input_csv_list=[data_yml["data"],'data/Demand.csv']
            stl_results = stl_decomposition(input_csv_list)
            best_distributions = analyze_distributions_with_fitter(stl_results)

            # Mostrar los mejores ajustes para cada componente
        for column, components in best_distributions.items():
            print(f"\nColumn: {column}")
